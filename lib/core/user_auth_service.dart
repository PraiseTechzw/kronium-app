...
class UserAuthService {
  ...
  Future<void> _loadUserProfile(String userId) async {
    try {
      ...
      if (user != null) {
        ...
        // Set role from user profile (don't hardcode as customer)
        if (user.role != null && user.role!.isNotEmpty) {
          print('DEBUG: Setting user role from database: ${user.role}');
          userController.setRole(user.role!);

          // Show role info toast for debugging
          Future.delayed(const Duration(milliseconds: 2000), () {
            if (user.role == 'admin') {
              print(
                'DEBUG: Admin user logged in successfully with role: ${user.role}',
              );
            } else {
              print('DEBUG: User logged in with role: ${user.role}');
            }
          });
        } else {
          print(
            'DEBUG: No role in database, defaulting to customer. User role was: ${user.role}',
          );
        ...
        if (supabaseUser != null) {
          try {
            final newUser = models.User(
              id: supabaseUser.id,
              simpleId:
                  null, // Will be auto-generated by database (format: AAA00001)
              name:
            ...
            if (createdUser != null) {
              ...
              // Set role from user profile (don't hardcode as customer)
              if (createdUser.role != null && createdUser.role!.isNotEmpty) {
                userController.setRole(createdUser.role!);
              } else {
                // Default to customer if no role is set
                userController.setRole('customer');
              }
      ...
      if (response.user != null) {
        ...
        try {
          ...
          return {
            'success': true,
            'message':
                'Registration successful, but profile creation failed. Please try logging in.',
            'user': response.user,
          };
        }
      } else {
  ...
  Future<void> forceGenerateSimpleId() async {
    try {
      ...
      if (userProfile.value!.simpleId == null ||
          userProfile.value!.simpleId!.isEmpty) {
        ...
        await _supabaseService.updateUser(currentUser.id, {
          'name':
              userProfile
                  .value!
                  .name, // Update with same value to trigger trigger
        });
        await _loadUserProfile(currentUser.id);
      }
    } catch (e) {
